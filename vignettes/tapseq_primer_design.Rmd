...
Let's start by creating sequence templates for primer design for a target gene panel. First we try
...
transcripts and add the "Beads-oligo-Dt" sequence used in single-cell droplet sequencing. Here we
use Bioconductor's BSgenome to export transcript sequences, but the genome sequence could also be
...
# create sequence templates for all truncated transcripts
seq_templates <- TAPseqSeqTemplates(truncated_txs, genome = hg38)
```
...
tamplates. The reverse primer used in all PCR reactions is used, so that Primer3 can pick primers
suitable to work with it. By default this function uses the 10x primer and chooses optimal, minimum
and maximum melting temperatures based on that. If another protocol (e.g. Drop-seq) is used, the
reverse primer and melting temperatures can be adapted (\code{?TAPseqInput}).
```{r, message=FALSE, warning=FALSE, results="hide"}
# create TAPseq IO for outer forward primers from sequence templates
outer_primers <- TAPseqInput(seq_templates, product_size_range = c(350, 500), primer_num_return = 5)
...
inner_primers <- TAPseqInput(seq_templates, product_size_range = c(150, 300), primer_num_return = 5)
